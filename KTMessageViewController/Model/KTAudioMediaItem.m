...
@interface KTAudioMediaItem ()

@property (strong, nonatomic) NSView *cachedMediaView;

@property (strong, nonatomic) NSButton *playButton;

@property (strong, nonatomic) NSProgressIndicator *progressView;
...
@implementation KTAudioMediaItem

#pragma mark - Initialization

- (instancetype)initWithData:(NSData *)audioData audioViewAttributes:(KTAudioMediaViewAttributes *)audioViewAttributes
{
...
{
    return [self initWithData:audioData audioViewAttributes:[[KTAudioMediaViewAttributes alloc] init]];
}
...
- (instancetype)init
{
    return [self initWithData:nil audioViewAttributes:[[KTAudioMediaViewAttributes alloc] init]];
}
...
- (void)onPlayButton:(NSButton *)sender
{
    //TODO:
...
//        [NSView transitionWithView:self.playButton
//                          duration:.2
...
- (NSView *)mediaView
{
    if (self.audioData && self.cachedMediaView == nil) {
        ...
        NSView * playView = [[NSView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, size.width, size.height)];
        //TODO:
...
//        playView.clipsToBounds = YES;
        ...
        self.playButton = [[NSButton alloc] initWithFrame:buttonFrame];
        //TODO:
...
//        [self.playButton addTarget:self action:@selector(onPlayButton:) forControlEvents:UIControlEventTouchUpInside];
        ...
        self.progressLabel = [[NSTextField alloc] initWithFrame:labelFrame];
        self.progressLabel.alignment = NSTextAlignmentLeft;
        ...
        self.progressLabel.stringValue = maxWidthString;
        ...
        self.progressLabel.stringValue = durationString;
        [playView addSubview:self.progressLabel];
        
        // TODO: create a progress bar
//        self.progressView = [[NSProgressIndicator alloc] initWithProgressViewStyle:UIProgressViewStyleDefault];
...
//        [playView addSubview:self.progressView];
        ...
        [KTMessagesMediaViewBubbleImageMasker applyBubbleImageMaskToMediaView:playView isOutgoing:self.appliesMediaViewMaskAsOutgoing];
...
- (BOOL)isEqual:(id)object
{
    ...
    KTAudioMediaItem *audioItem = (KTAudioMediaItem *)object;
...
{
    KTAudioMediaItem *copy = [[[self class] allocWithZone:zone] initWithData:self.audioData
