...
/**
 *  The `KTVideoMediaItem` class is a concrete `KTMediaItem` subclass that implements the `KTMessageMediaData` protocol
 *  and represents a video media message. An initialized `KTVideoMediaItem` object can be passed
 *  to a `KTMediaMessage` object during its initialization to construct a valid media message object.
 *  You may wish to subclass `KTVideoMediaItem` to provide additional functionality or behavior.
 */
@interface KTVideoMediaItem : KTMediaItem <KTMessageMediaData, NSCoding, NSCopying>
 ...
 *
 *  @return An initialized `KTVideoMediaItem` if successful, `nil` otherwise.
 *
 *  @discussion If the video must be downloaded from the network,
 *  you may initialize a `KTVideoMediaItem` with a `nil` fileURL or specify `NO` for
